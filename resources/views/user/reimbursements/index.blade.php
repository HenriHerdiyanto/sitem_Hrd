                            ...
                            <div class="card-header">
                                <div class="row">
                                    ...
                                    <div class="col-4 col-sm-4 d-flex justify-content-end">
                                        <a href="{{ route('user.reimbursements.create') }}"
                                            ...
                                            @foreach ($reimbursements as $data)
                                                <tr>
                                                    ...
                                                    <td class="text-center text-nowrap">
                                                        @if ($data->persetujuan_finance == 'diproses')
                                                            <form
                                                                action="{{ route('user.reimbursements.destroy', $data->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('post')
                                                                <a class="btn btn-sm btn-warning"
                                                                    href="{{ route('user.reimbursements.edit', $data->id) }}">Edit</a>
                                                                <button type="submit"
                                                                    class="btn btn-sm btn-danger">Hapus</button>
                                                            </form>
                                                        @elseif ($data->persetujuan_finance == 'diterima')
                                                            <a class="btn btn-sm btn-warning"
                                                                href="{{ route('user.reimbursements.cetak', $data->id) }}">cetak</a>
                                                        @elseif ($data->persetujuan_finance == 'ditolak')
                                                            <a class="btn btn-sm btn-warning"
                                                                href="{{ route('dinas.edit', $data->id) }}">Lihat</a>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
